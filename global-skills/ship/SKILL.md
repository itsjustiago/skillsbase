...
**Do NOT use when:**
- The user asks for a review, tests, or docs update first → do that first (`/code-review`, testes, `/verify`), depois volta ao ship
...
- Write a **Conventional Commit** message from the staged diff: `type(scope): summary` (feat/fix/refactor/chore/docs…), imperative, ≤72 chars na primeira linha; body em bullets só se o diff for grande. Segue o estilo visível em `git log -5`.
...
**If no PR exists:** generate title + body from the branch's commits and diff against the base branch (usually `main`), following the template below.
...
| Commit msg | Write from the diff | Conventional Commits, estilo do `git log -5` |
...
| PR body | Generate from commits + diff | Template acima |
...
| Mistake | Fix |
|---------|-----|
| Commit message vaga ("update", "fixes") | Deriva do diff: tipo + scope + resumo concreto |
...
- User asked for "ship with tests" or "ship with review" → corre primeiro `/code-review` / testes / `/verify`, depois ship
...
…you are using the wrong skill. Para o fluxo completo: `/code-review` + testes + `/verify` primeiro, e só depois `/ship` ou `/ship-merge`.
